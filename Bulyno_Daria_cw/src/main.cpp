	...
	while (generation == "y" || generation == "yes") {
		...
		if (operation == "0") break;
			...
			if (!newtree) cout << "Данного элемента нет в дереве.\n";
				...
				cout << endl << "Количество итераций для поиска элемента: " << iterations << endl;
			...
			if (find(tree, value)) { //проверка на наличие элементав дереве
...
void func_for_researhing_average(int size, int new_iterations, string generation, Node* tree, Node* btree, Node* newtree, vector<int> average_case) {
	...
	end1 = chrono::system_clock::now(); //конец подсчета времени для сл. БДП с рандомизацией
	...
	end2 = chrono::system_clock::now(); //конец подсчета времени для сл. БДП
	chrono::duration<double> duration2 = end2 - start2; //промежуток времени работы функции для сл. БДП
	cout << "Случайное БДП в среднем случае для сравнения:" << endl;
...
int main() {
	...
	string menu_for_generation; //используется в качестве меню для генерации деревьев
	string menu_for_creation; //используется в качестве меню по обработке созданного дерева
	...
	while (start_end == "y" || start_end =="yes"){
		int size;
		cout << "Выберите действие, которое хотите осуществить:\n";
		cout << "0. Завершить работу.\n1. Сгенерирровать случайное БДП с рандомизацией и провести его анализ.\n2. Построить случайное БДП с рандомизацией.\n";
		...
		if(menu == "0") break;
			...
			while (menu_for_creation == "y" || menu_for_creation == "yes") { //меню выбора метода работы с деревом
				...
				if (method == "0") break;
					...
					else cout << "Данного элеента нет в дереве.\n";
				}
				else{
					cout << "Действие выбрано неверно.";
				}
